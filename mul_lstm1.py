...
def fun(TIME_STEPS,
    ...
    #train_data = data[0:math.floor(len(data)*0.8)]


    train_end_index = math.floor(len(data) * 0.5)
    ...
    class LSTMRNN(object):
        ...
        def add_cell(self):
            lstm_cell = tf.nn.rnn_cell.BasicLSTMCell(self.cell_size, forget_bias=0.0, state_is_tuple=True)
    ...
    if is_test:
        ...
        acc3 = np.average(np.abs(test_predict - test_target[:len(test_predict)]) / test_target[:len(test_predict)])
        print("总的相对误差:", acc3)

        return [test_predict,test_target,1,1,acc3]
